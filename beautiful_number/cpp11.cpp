...
// превосходит m
template <typename T>
auto composition(T n, T k, T m) -> T {
    if (n == 0 && k == 0) {
        return 1;
    }
    if (n > k * m) {
...
template <typename T>
auto beautiful_number(T len, T num) -> T {
    auto count = T{};
    auto k = len / 2;
    auto m = num - 1;
...
int main(int argc, char *argv[]) {
    std::cout << std::to_string(beautiful_number<std::uint64_t>(13, 13)) << std::endl;
    return 0;
}
